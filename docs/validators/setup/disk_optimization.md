...
speed and transaction amount. For bitos, we are talking about close to
...
I believe this was disabled by default on bitos, but listing it in any
...
will risk database corruption if the `bitosd` is killed for any reason.
...
Below is the disk usage after two weeks of bitos Arsia Mons testnet. The default
