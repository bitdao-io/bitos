...
`TextProposal`s are used by delegators to agree to a certain strategy, plan, commitment, future upgrade, or any other statement in the form of text. Aside from having a record of the proposal outcome on the bitos chain, a text proposal has no direct effect on bitos.

#### Real Example

[Proposal 1](https://commonwealth.im/bitos/proposal/1-airdrop-claim-mission) was representative of one of four core network activities that users had to participate in to claim tokens from the bitos Rektdrop.
...
3. **Recipient** - the bitos (bech32-based) address that will receive funding from the Community Pool
4. **Amount** - the amount of funding that the recipient will receive in atto-bitos (`abitos`)
5. **Deposit** - the amount that will be contributed to the deposit (in `abitos`) from the account submitting the proposal

#### Made-Up Example

In this simple example (below), a network explorer will list the governance proposal as a `CommunityPoolSpendProposal`. When an observer selects the proposal, they'll see the description. Not all explorers will show the recipient and amount, so ensure that you verify that the description aligns with the what the governance proposal is programmed to enact. If the description says that a certain address will receive a certain number of bitos, it should also be programmed to do that, but it's possible that that's not the case (accidentally or otherwise).

The `amount` is `1000000000000000000abitos`. This is equal to 1 bitos, so `recipient` address `bitos1mx9nqk5agvlsvt2yc8259nwztmxq7zjq50mxkp` will receive 1 bitos if this proposal is passed.

The `deposit` of `64000000000000000000abitos` results in 64 bitos being used from the proposal submitter's account. There is a minimum deposit required for a proposal to enter the voting period, and anyone may contribute to this deposit within a 5-day period. If the minimum deposit isn't reached before this time, the deposit amounts will be burned. Deposit amounts will also be burned if quorum isn't met in the vote or if the proposal is vetoed.
...
```json
{
  ...
  "recipient": "bitos1mx9nqk5agvlsvt2yc8259nwztmxq7zjq50mxkp",
  "amount": [
    {
      "denom": "abitos",
...
This is a governance protocol which [Flux Protocol](https://www.fluxprotocol.org/), the provider of a cross-chain oracle which provides smart contracts with access to economically secure data feeds, submitted to cover costs of the subsidizied FPO (First Party Oracle) solution which they deployed on the bitos mainnet.

Users can query the proposal details with the `bitosd` command-line interface using this command:

```bash
`bitosd --node https://tendermint.bd.bitos.org:26657 query gov proposal 23`.
```

```json
{
  "title": "Grant proposal for Flux Protocol an oracle solution live on bitos",
  "description": "proposal: https://gateway.pinata.cloud/ipfs/QmfZknL4KRHvJ6XUDwtyRKANVs44FFmjGuM8YbArqqfWwF discussion: https://commonwealth.im/bitos/discussion/4915-bitos-grant-flux-oracle-solution"
  "recipient": "bitos15dxa2e3lc8zvmryv62x3stt86yhplu2vs9kxct",
  "amount": [
    {
...
1. **Title** - the distinguishing name of the proposal, typically the way the that explorers list proposals
2. **Description** - the body of the proposal that further describes what is being proposed and details surrounding the proposal
3. **Subspace** - the bitos module with the parameter that is being changed
...
6. **Denom** - `abitos` (atto-bitos) will be the type of asset used as the deposit
7. **Amount** - the amount that will be contributed to the deposit (in `abitos`) from the account submitting the proposal

#### Real Example

In the example below, a network explorer listed the governance proposal by its title: "Increase the minimum deposit for governance proposals." When a user selects the proposal, they'll see the proposal’s description. This proposal can be [found on the bitos network here](https://commonwealth.im/bitos/proposal/7-increase-the-minimum-deposit-for-governance-proposals).
...
```bash
`bitosd --node https://tendermint.bd.bitos.org:26657 query gov proposal 7`.
```

```json
{
  "title": "Increase the minimum deposit for governance proposals",
  "description": "If successful, this parameter-change governance proposal will change the minimum deposit for future proposals from 10 bitos tokens to 64.",
  "changes": [
    {
      "subspace": "gov",
      "key": "depositparams",
      "value": {"mindeposit":[{"denom":"abitos","amount":"64000000000000000000"}],
...
The deposit `denom` is `abitos` and `amount` is `20100000000000000000`. Therefore, a deposit of 20.1 bitos will be included with this proposal. At the time, the bitos mainnet had a 10 bitos minimum deposit, so this proposal was put directly into the voting period (and subsequently passed). There is a minimum deposit required for a proposal to enter the voting period, and anyone may contribute to this deposit within a 5-day period. If the minimum deposit isn't reached before this time, the deposit amounts will be burned.

## Sending the transaction that submits your governance proposal

For information on how to use `bitosd` binary to submit an on-chain proposal through the governance module, please refer to the [quickstart](../quickstart/binary.md) documentation.

### CLI

This is the command format for using `bitosd` (the command-line interface) to submit your proposal on-chain:

```bash
bitosd tx gov submit-proposal \
  --title=<title> \
  --description=<description> \
  --type="Text" \
  --deposit="1000000abitos" \
  --from=<mykey> \
  --chain-id=<chain_id>
  --node <address>
```
...
::: tip
Use the `bitos tx gov --help` flag to get more info about the governance commands
:::

1. `bitosd` is the command-line interface client that is used to send transactions and query bitos
...
5. `--fees` is a flat-rate incentive for a validator to process your transaction
   ...
   - 7500abitos is equal to 0.0075 bitos
6. `--chain-id bitos_90001-2` is bitos Mainnet. For current and past chain-id's, please look at the [tharsis/mainnet resource](https://github.com/tharsis/mainnet)
   - the testnet chain ID is [bitos_9000-4](https://testnet.mintscan.io/bitos). For current and past testnet information, please look at the [testnet repository](https://github.com/tharsis/testnets)
7. `--node` is using a full node to send the transaction to the bitos Mainnet

### Verifying your transaction

After posting your transaction, your command line interface (`bitosd`) will provide you with the transaction's hash, which you can either query using bitosd or by searching the transaction hash using [Mintscan](https://www.mintscan.io/bitos) or any block explorer.
...
```bash
bitosd q gov proposals --depositor bitos1hxv7mpztvln45eghez6evw2ypcw4vjmsmr8cdx
```

Once you have the proposal ID, this is the command to deposit extra tokens:

```bash
bitosd tx gov deposit <proposal-id> <deposit> --from <name>
```

In our case above, the `<proposal-id>` would be 59 as queried earlier.
The `<deposit>` is written as `500000abitos`, just like the example above.
...
- the deposit denomination is in 'atbitos' instead of 'abitos'
