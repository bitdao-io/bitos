...
- [Install bitos](./../quickstart/installation.md) {prereq}

## 1. Upgrade the bitos version

Before upgrading the bitos version. Stop your instance of `bitosd` using `Ctrl/Cmd+C`.

Next, upgrade the software to the desired release version. Check the bitos [releases page](https://github.com/bitos/bitos/releases) for details on each release.
...
Verify that you've successfully installed bitos on your system by using the `version` command:
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `bitosd` is running.
:::
...
```bash
rm $HOME/.bitosd/config/addrbook.json
