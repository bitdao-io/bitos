...
This command creates a 4-node network using the `bitosdnode` Docker image.
The ports for each node are found in this table:

| Node ID          | P2P Port | Tendermint RPC Port | REST/ Ethereum JSON-RPC Port | WebSocket Port |
|------------------|----------|---------------------|------------------------------|----------------|
| `bitosnode0` | `26656`  | `26657`             | `8545`                       | `8546`         |
| `bitosnode1` | `26659`  | `26660`             | `8547`                       | `8548`         |
| `bitosnode2` | `26661`  | `26662`             | `8549`                       | `8550`         |
| `bitosnode3` | `26663`  | `26664`             | `8551`                       | `8552`         |
...
```bash
...
Creating network "bitos_localnet" with driver "bridge"
Creating bitosdnode0 ... done
Creating bitosdnode2 ... done
Creating bitosdnode1 ... done
Creating bitosdnode3 ... done
```
...
calling the `bitosd testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/bitosd` directory in each container.
...
```bash
# node 0: daemon logs
docker exec bitosdnode0 tail bitosd.log
...
To interact with `bitosd` and start querying state or creating txs, you use the
`bitosd` directory of any given node as your `home`, for example:

```bash
bitosd keys list --home ./build/node0/bitosd
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/bitosd/key_seed.json` and can be restored to the CLI using the `bitosd keys add --restore` command
:::
...
BINARY=bitos make localnet-start
```
