...
const (
	// MainnetChainID defines the Bitos EIP155 chain ID for mainnet
	MainnetChainID = "bitos_9001"
	// TestnetChainID defines the Bitos EIP155 chain ID for testnet
	TestnetChainID = "bitos_9000"
)

// IsMainnet returns true if the chain-id has the Bitos mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Bitos testnet EIP155 chain prefix.
...
// GetBitosAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `bitos`).
// The function fails if the provided bech32 address is invalid.
func GetBitosAddressFromBech32(address string) (sdk.AccAddress, error) {
