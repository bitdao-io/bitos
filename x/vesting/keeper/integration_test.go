...
import (
	...
	"github.com/bitdao-io/bitos/testutil"
	...
	"github.com/bitdao-io/bitos/x/vesting/types"
)
...
func delegate(clawbackAccount *types.ClawbackVestingAccount, amount int64) error {
	...
	s.Require().NoError(err)
	//
	val, err := sdk.ValAddressFromBech32("bitvaloper1z3t55m0l9h0eupuz3dp5t5cypyv674jjn4d6nn")
	s.Require().NoError(err)
