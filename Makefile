...
SIMAPP = ./app
HTTPS_GIT := https://github.com/bitdao-io/bitos.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=bitos \
...
build-docker:
	...
	$(DOCKER) rm bitos || true
	# create a new container from the latest image
	$(DOCKER) create --name bitos -t -i ${DOCKER_IMAGE}:latest bitos
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp bitos:/usr/bin/bitosd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/bitdao-io/bitos/types"
	godoc -http=:6060
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/bitdao-io/bitos
.PHONY: format
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(BITOS_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\bitos\Z bitosd/node "./bitosd testnet --v 4 -o /bitos --keyring-backend=test --ip-addresses bitosdnode0,bitosdnode1,bitosdnode2,bitosdnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(BITOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/bitos:Z bitosd/node "./bitosd testnet --v 4 -o /bitos --keyring-backend=test --ip-addresses bitosdnode0,bitosdnode1,bitosdnode2,bitosdnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\bitosd:bitos\Z bitosd/node "./bitosd unsafe-reset-all --home=/bitos"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\bitosd:bitos\Z bitosd/node "./bitosd unsafe-reset-all --home=/bitos"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\bitosd:bitos\Z bitosd/node "./bitosd unsafe-reset-all --home=/bitos"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\bitosd:bitos\Z bitosd/node "./bitosd unsafe-reset-all --home=/bitos"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/bitosd:/bitos:Z bitosd/node "./bitosd unsafe-reset-all --home=/bitos"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/bitosd:/bitos:Z bitosd/node "./bitosd unsafe-reset-all --home=/bitos"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/bitosd:/bitos:Z bitosd/node "./bitosd unsafe-reset-all --home=/bitos"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/bitosd:/bitos:Z bitosd/node "./bitosd unsafe-reset-all --home=/bitos"
endif
...
.PHONY: build-docker-local-bitos localnet-start localnet-stop
