...
## default port prefixes for bitosd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/bitosd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/bitosd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/bitosd add-genesis-account \
    "$("$PWD"/build/bitosd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000abit,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/bitosd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/bitosd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/bitosd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/bitosd start --pruning=nothing --rpc.unsafe \
    ...
    BITOS_PID=$!
    echo "started bitos node, pid=$BITOS_PID"
    # add PID to array
    arr+=("$BITOS_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test bitos node $HOST_RPC ..."
